<div class="vi-pdf-card">
    <div class="vi-card-header">
        ...
        <a href="{{ route($file->media->first()->collection_name) }}" class="vi-category" wire:navigate>{{ ucfirst($file->media->first()->collection_name) }}</a>
    </div>
    ...
    <div class="vi-card-footer">
        ...
        <a href="{{ route('view-file', ['media' => $file->media->first()]) }}" class="flex items-center vi-downloads-links source-file" target="_blank">
            <span>View File</span>
        </a>
    </div>
</div>
